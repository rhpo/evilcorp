    ...
    public partial class BruteForceForm : Form
    {
        ...
        private void PaintHeader(object sender, PaintEventArgs e)
        {
            ...
            using var bgGrad = new LinearGradientBrush(
                ...
                Color.FromArgb(60, AccentBlue), Color.FromArgb(0, AccentBlue));
            g.FillRectangle(bgGrad, 0, 0, w, h);

            // Top accent line
            g.DrawLine(new Pen(Color.FromArgb(160, AccentCyan), 2), 0, 0, w * 2 / 3, 0);

            // Bottom border
            g.DrawLine(new Pen(Color.FromArgb(28, 45, 80), 1), 0, h - 1, w, h - 1);

            using var titleFont = new Font("Segoe UI", 24f, FontStyle.Bold);
            g.DrawString("Brute Force Attack", titleFont,
                new SolidBrush(TextPrimary), new PointF(28, 24));

            using var subFont = new Font("Segoe UI", 10f);
            g.DrawString("Exhaustive charset enumeration  |  EvilCorp Security Suite", subFont,
        ...
        private void PaintCard(object sender, PaintEventArgs e)
        {
            ...
            using var bar = new LinearGradientBrush(
                ...
                AccentBlue, Color.FromArgb(5, AccentBlue));
            g.FillRectangle(bar, 0, 0, 4, h);

            g.DrawRectangle(new Pen(CardBorder, 1), 0, 0, w - 1, h - 1);
            g.FillEllipse(new SolidBrush(Color.FromArgb(9, AccentBlue)), w - 320, h - 320, 460, 460);

            using var cornerPen = new Pen(Color.FromArgb(35, AccentCyan), 1);
        ...
        private static void StyleTextBox(TextBox tb)
        {
            ...
            tb.Font = new Font("Consolas", 10.5f);
        }
        ...
        {
            btn.Text = "⚡  LAUNCH ATTACK";
            btn.Font = new Font("Segoe UI", 11f, FontStyle.Bold);
            btn.ForeColor = Color.FromArgb(210, 228, 255);
            btn.BackColor = Color.FromArgb(14, 32, 78);
            btn.FlatStyle = FlatStyle.Flat;
            btn.FlatAppearance.BorderSize = 1;
            btn.FlatAppearance.BorderColor = Color.FromArgb(55, 115, 220);
            btn.FlatAppearance.MouseOverBackColor = Color.FromArgb(22, 50, 118);
        ...
        private async void btnStart_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(charset)) { MessageBox.Show("Charset cannot be empty."); return; }
            if (minLen > maxLen) { MessageBox.Show("Min length cannot be greater than max length."); return; }

            _cts = new CancellationTokenSource();
            btnStart.Enabled = false;
            btnStop.Enabled = true;
            progress.Value = 0;
            lblStatus.Text = "[ RUNNING ]  Brute force in progress...";
            lblStatus.ForeColor = AccentOrange;

            await RunBruteForce(targetPassword, charset, minLen, maxLen, _cts.Token);

            if (!_accountLocked) { btnStart.Enabled = true; btnStop.Enabled = false; }
        }

        private async Task RunBruteForce(string targetPassword, string charset,
            ...
            foreach (var candidate in GenerateCombinations(charset, minLength, maxLength))
            {
                ...
                if (candidate.Equals(targetPassword, StringComparison.Ordinal))
                {
                    ...
                    MessageBox.Show($"Password found:  {candidate}\n\nAttempts : {attempts:N0}\nTime : {sw.Elapsed.TotalMilliseconds:N2} ms",
                        "Brute Force - Cracked!", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                ...
                if (attempts % 200 == 0)
                {
                    lblStatus.Text = $"[ RUNNING ]  Testing: {candidate}   ({attempts:N0} attempts)";
                    progress.Value = (attempts / 200) % 101;
                    await Task.Delay(1, token).ContinueWith(_ => { });
                }
            }

            sw.Stop();
            lblStatus.Text = "[ FAILED ]  Password not found.";
            lblStatus.ForeColor = AccentRed; btnStop.Enabled = false;
            MessageBox.Show($"Password not found.\n\nAttempts : {attempts:N0}\nTime : {sw.Elapsed.TotalMilliseconds:N2} ms",
                "Brute Force - Failed", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
        ...
        private IEnumerable<string> GenerateCombinations(string charset, int minLength, int maxLength)
        {
            for (int length = minLength; length <= maxLength; length++)
                foreach (var r in Generate(charset, "", length))
                    yield return r;
        }

        private IEnumerable<string> Generate(string charset, string prefix, int remaining)
        {
            if (remaining == 0) { yield return prefix; yield break; }
            foreach (char c in charset)
                foreach (var r in Generate(charset, prefix + c, remaining - 1))
                    yield return r;
        }
    }
}